    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(int64_t k, int64_t j) {
            zadat_view(k, j) =
                (zpdat_view(k + 1, j - 1) + zqdat_view(k + 1, j - 1) -
                 zpdat_view(k, j - 1) - zqdat_view(k, j - 1)) *
                (zrdat_view(k, j) + zrdat_view(k, j - 1)) /
                (zmdat_view(k, j - 1) + zmdat_view(k + 1, j - 1));

            zbdat_view(k, j) = (zpdat_view(k, j - 1) + zqdat_view(k, j - 1) -
                                zpdat_view(k, j) - zqdat_view(k, j)) *
      ...
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(int64_t k, int64_t j) {
            zudat_view(k, j) +=
                s *
                (zadat_view(k, j) * (zzdat_view(k, j) - zzdat_view(k, j + 1)) -
                 zadat_view(k, j - 1) *
                     (zzdat_view(k, j) - zzdat_view(k, j - 1)) -
                 zbdat_view(k, j) * (zzdat_view(k, j) - zzdat_view(k - 1, j)) +
                 zbdat_view(k + 1, j) *
                     (zzdat_view(k, j) - zzdat_view(k + 1, j)));
            zvdat_view(k, j) +=
                s *
                (zadat_view(k, j) * (zrdat_view(k, j) - zrdat_view(k, j + 1)) -
                 zadat_view(k, j - 1) *
                     (zrdat_view(k, j) - zrdat_view(k, j - 1)) -
